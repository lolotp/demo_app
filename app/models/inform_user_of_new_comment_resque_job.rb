class InformUserOfNewCommentResqueJob
  ...
  def self.create_notification_for_receiver(receiver, commenter, post, comment)
    receiver.notifications.build( :content => "<n><a href=\"memcap://users/#{commenter.id}\" >#{commenter.name}</a> commented on your <a href=\"memcap://posts/#{post.id}\">post</a> <a href=\"memcap://comments/#{comment.id}\"/></n>", :viewed => false)
  end

  def self.create_notification(commenter, post, comment)
    Notification.new( :content => "<n><a href=\"memcap://users/#{commenter.id}\" >#{commenter.name}</a> commented on a <a href=\"memcap://posts/#{post.id}\">post</a> you commented on. <a href=\"memcap://comments/#{comment.id}\"/></n>", :viewed => false)
  end
  ...
  def self.perform(comment_id, at_time, should_inform_owner)
    ...
    if (commenter.id != post_owner.id and should_inform_owner)
      notification = self.create_notification_for_receiver(post_owner, commenter, post, comment)
    ...
    begin
      ...
      comments_from_unqiue_users.each do |comment|
        notification = self.create_notification(commenter, post, comment)
