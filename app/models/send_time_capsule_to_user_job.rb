...
class SendTimeCapsuleToUserJob
  ...
  def self.perform(from_user_id, to_user_id, post_id)
    ...
    if from_user.friend?(to_user)
      ...
      notification = to_user.notifications.build( :content => "<n><a href=\"memcap://users/#{from_user.id}\" >#{from_user.name}</a> sent you a <a href=\"memcap://posts/#{post.id}\">time capsule</a>", :viewed => false)
